  ...
  socket.on('new_connection', (data) => {
    ...
    try {
      ...
      if (room_connections.length == 0) {
        ...
        rooms.find(room => room.id == data.room).host = data.username;
      } else {
        data.host = false;
      }
      data.admin = false;
 ...
 socket.on("disconnect", (reason, details) => {
  ...
  try {
    ...
    try {
      ...
      connections_in_room = connections.filter(ele => ele.room == found.room);
      ...
      if (found.host && connections_in_room.length > 0) {
        ...
        if (admins.length > 0) {
          admins[0].host = true
          rooms.find(ele => ele.id == found.room).host = admins[0].username;
          io.to(admins[0].socket_id).emit('make_host')
          console.log(`changed host to`,admins[0].username)
          socket.to(found.room).emit('receive_server_message',{
          ...
          rooms.find(ele => ele.id == found.room).host = connections_in_room[0].username;
