...
function setup() {
    ...
    undo.mouseClicked(() => {
        ...
        p5layers[currentLayer]["live"].set(p5layers[currentLayer]["states"].pop());
    ...
    p5layers.forEach(layer => {
        ...
        option.innerText = `Layer ${layer.id}`;
    ...
    document.querySelector(`[data-action="new-layer"]`).addEventListener("click", (event) => {
        try {
            if (p5layers.length >= MAX_LAYERS) {
                throw new Error(`Add Layer: At max layers.`)
            }
            // get max id
            let max = 1;
            p5layers.forEach(layer => {
...
function deleteLayer(number) {
    try {
        if (number <= 0) {
            throw new Error('Cannot delete background or negative number.')
        }
        // find
        let found = p5layers.find((element) => element.id == number),
            to_be_removed = p5layers.indexOf(found);
        // cleanup
        found.live.remove();
        found.states.forEach(state => { state.remove(); });
        found.baked.remove();
        // kill
        p5layers.splice(to_be_removed,1);
        // success!
        console.log(`Delete Layer: Success! Removed Layer ${number}`)
    } catch(err) {
        console.log(`Delete Layer: ${err}`);
    }
}
...
function finalize_action(action) {
    ...
    let state = p5layers[action[0].layer]["live"].get();
    console.log(`layer: ${action[0].layer}?`) // createGraphics(cWidth,cHeight,p5layers[action[0].layer]["live"]);
